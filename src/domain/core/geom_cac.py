...
class GeomCac:
    ...
    def get_emit_refPnts(cls, mobject, ex_in, geom, void_area, emit_debug = False, tol=1e-9):
        ...
        def resolve_geom_points(name):
            if name in geom:
                return geom[name]['points']
            for bucket in ("areas", "lines", "points"):
                group = geom.get(bucket, {})
                if name in group:
                    value = group[name]
                    if bucket == "points":
                        return [value]
                    return value.get("points", value)
            raise KeyError(name)

        # 1. 分割字符串为数字列表
        mobject_pnts = resolve_geom_points(mobject)
        ...
        if len(ex_in) >= 2:
            ...
            for i in range(0, len(ex_in), 2):
                ...
                if ex_in_kind in ("EXCLUDE", "INCLUDE"):
                    ex_in_area_pnts = resolve_geom_points(ex_in_area_name)
