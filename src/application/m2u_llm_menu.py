...
class MAGIC2UNIPIC:
    ...
    def set_input_file(self, input_file: str):
        ...
        self.uni_symbols = Unipic25dSymbolTable()
        self.llmconv_list = []
        self.mcl2mid_conv = MCL2MIDST_LLMConv(
    ...
    def _load_mid_symbols(self, path: Path):
        ...
        self._normalize_mid_symbol_geometry(mid_symbols.sT)
        self.mid_symbols = mid_symbols
        return mid_symbols

    def _normalize_mid_symbol_geometry(self, st: dict):
        geometry = st.get("geometry", {})

        for point_info in geometry.get("point", {}).values():
            geom_num = point_info.get("cac_result", {}).get("geom_num")
            if isinstance(geom_num, list) and len(geom_num) == 2 and not any(isinstance(x, (list, dict)) for x in geom_num):
                point_info["cac_result"]["geom_num"] = tuple(geom_num)

        for line_info in geometry.get("line", {}).values():
            geom_num = line_info.get("cac_result", {}).get("geom_num")
            if isinstance(geom_num, list):
                line_info["cac_result"]["geom_num"] = [
                    tuple(pt) if isinstance(pt, list) else pt
                    for pt in geom_num
                ]

        for area_info in geometry.get("area", {}).values():
            geom_num = area_info.get("cac_result", {}).get("geom_num")
            if isinstance(geom_num, list):
                area_info["cac_result"]["geom_num"] = [
                    tuple(pt) if isinstance(pt, list) else pt
                    for pt in geom_num
                ]

        area_cac_result = geometry.get("area_cac_result", {})
        void_area = area_cac_result.get("void_area", {})
        pnts = void_area.get("pnts")
        if isinstance(pnts, list):
            void_area["pnts"] = [tuple(pt) if isinstance(pt, list) else pt for pt in pnts]

        other_entity = area_cac_result.get("other_entity", {})
        for entity_info in other_entity.values() if isinstance(other_entity, dict) else []:
            pnts = entity_info.get("pnts")
            if isinstance(pnts, list):
                entity_info["pnts"] = [tuple(pt) if isinstance(pt, list) else pt for pt in pnts]
    ...
    def step_convert_round1(self):
        ...
        self._write_json(self.constants.parsed_json, self.magic_symbols.cmds_list)
        self._write_json(self.constants.mid_symbol1_json, self.mid_symbols.to_dict())
        self._write_json(self.constants.llmconv_json, self.llmconv_list)
...
def main():
    parser = argparse.ArgumentParser(description="LLM 流程菜单版 MAGIC2UNIPIC")
