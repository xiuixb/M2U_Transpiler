...
from src.core_symbol.muti_flows.llm_conv.llm_conv import LLMConv
from src.config.llm_config import llm_config
from src.core_cac.cac_flows import MCLPromptBuildFlow
from src.core_cac.mcl2mid_dict import MCL2MID_Dict
...
class MCL2MID_STConv:
    ...
    def process_record(self, idx: int, record: dict):
        ...
        if self.llmconv_debug:
            if kind not in ("variable", "function", "point", "line", "area"):
            ...
            if kind in ("mark", "material_assign", "port", "emission", "emit", "preset", "inductor", "resistor", "foil"):
                print(f"       LLM处理的条目类型: {cmd_type}")
                cmd_type_single = cmd_type.split()[0]
                shouldConvert = cmd_type_single in self.MCL2MID_dict
                if shouldConvert:
    ...
    def _process_variable(self, idx: int, record: dict):
        ...
        try:
            ...
            substituted_expr = substitute_variables(expr_str, self.mid_symbols.sT["variable"])  # 替换变量名为符号表中对应的值和单位
