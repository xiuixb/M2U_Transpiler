...
from src.core_cac.unit_tool import UnitTool
...
class MID2UNI_STConv():
    ...
    def mid2uni_sTconv(self,
        ...
        # print(json.dumps(self.mid_symbols.test_dict(), indent=2, ensure_ascii=False))
        ### 1. 保存变量表
        symbols = []
        for name, info in self.mid_symbols.sT["variable"].items():
            symbols.append({
                'name': name,
                'value': info["cac_result"]['var_num'],
                'unit': info["cac_result"]['var_unit']
            }) 
        ...
        self.mid2FaceBndIn(
                    port_num_dict = port_num_dict,
                        ...
                        excitation_info = self.mid_symbols.sT["physics_entities"]["field_excitation"],
                    )
        ...
        ### 7. FieldsDgn.in
        if self.mid_symbols.sT["diagnostic"]:
            geo_c = self.mid2FieldsDgnIn(geo_c)

        ### 8. StaticNodeFLds.in
        rMax = self.GetrMax()
        if self.mid_symbols.sT["physics_entities"]["electromagnetic_field"] is not None:
            self.mid2StaticNodeFLdsIn(rMax = rMax)

        
        ### 9. CircuitModel.in
        if self.mid_symbols.sT["physics_entities"]["inductor"] is not None:
            # print(f"[info] {self.mid_symbols.inductor}")
            self.mid2CircuitModelIn()

        ### 10. FoilModel.in
        if self.mid_symbols.sT["physics_entities"]["foil_model"] is not None:
    ...
    def mid2buildIn(self,
        ...
        if IF_Conv2Void == True:
            ...
            # print(self.mid_symbols.sT["geometry"]["area_cac_result"])
            
            for point in self.mid_symbols.sT["geometry"]["area_cac_result"]['void_area']["pnts"]:
                x, y = point
                if axis_mcl_dir == 'X':
    ...
    def mid2portSelection(self,
        ...
        for port_index, port in enumerate(self.mid_symbols.sT["boundaries"]['port']):
            port_info = port['parameters']
            port_direction = port_info['direction']
            if port_direction == 'POSITIVE':
                ...
                self.mid_symbols.sT["boundaries"]['port'][port_index]['cac_result']['inputMurPort_refPnt'] = refPnt
                ...
                positive_port_info = port
                theGeomCtrl_list.append(selection_builder)
                
            
            elif port_direction == 'NEGATIVE':
                port_geom_value = port_info['geom_value']
                coord1, coord2 = port_geom_value
                midpoint = [(a + b) / 2 for a, b in zip(coord1, coord2)]
                refPnt = f"[{midpoint[0]} {midpoint[1]} {midpoint[2]}]"
                self.mid_symbols.sT["boundaries"]['port'][port_index]['cac_result']['openPort_refPnt'] = refPnt
    ...
    def mid2emitSelection(self,
        ...
        emit_num_list = []
        emit_kind = ""
        for emit in self.mid_symbols.sT["physics_entities"]['emit_apply']:
            ...
            refPnts = emit["cac_result"]["emit_selection_refPnts"]
            #print(f"emit['cac_result'] = {emit['cac_result']}")
            ...
            for refPnt in refPnts:
                ...
                selection_builder = {
                    ...
                    "content":[{
                        ...
                        "mask": emitter_mask_num, 
                        "name": "selection" + str(selection_num),
                        "refPnt": refPnt, 
    ...
    def mid2GridDefine(self, 
        ...
        if axis_mcl_dir == 'X':
            z_min = self.mid_symbols.sT["mesh"].get('X1')
            y_min = self.mid_symbols.sT["mesh"].get('X2')
        elif axis_mcl_dir == 'Y':
            # 如果以后支持 Y 为主轴，可按需要调整映射
            z_min = self.mid_symbols.sT["mesh"].get('X2')
            y_min = self.mid_symbols.sT["mesh"].get('X1')
        else:
            z_min = self.mid_symbols.sT["mesh"].get('X1')
            y_min = self.mid_symbols.sT["mesh"].get('X2')
    ...
    def mid2FaceBndIn(self,
                      port_num_dict: dict,
                      port_info: dict,
                      excitation_info: dict,
                      ):
        print(f"[info] Saving FaceBndInIn")
        print(f"       port_info = {port_info}")
        
        sys_no = port_info['sys_no']
        exc_info = None
        for exc in excitation_info:
            if exc['port_no'] == sys_no:
                exc_info = exc
                break
        else:
            raise ValueError(f"未找到与端口 {sys_no} 匹配的激励信息")
        print(f"       excitation_info = {exc_info}")
        
        positive_port_mask_num = port_num_dict['positive_port_mask_num']
        FieldSrc_Function_kind = "tFunc"
        FieldSrc_Function_result = exc_info['parameters']['incoming_func_body']
        FieldSrc_Function_args = exc_info['parameters']['incoming_func_vars']

        # 写入FieldSrc
        FieldSrc_para_dict = {
            "sys_type":"ini",
            "mask": positive_port_mask_num, 
            "kind": "MurVoltagePort", 
            "fileName": "Default.h5"
        }
        ...
        FieldSrc_Function_dict.update({
            "kind": FieldSrc_Function_kind, 
            "result": FieldSrc_Function_result
        })
    ...
    def mid2PtclSourcesIn(self,                          
        ...
        print(f"       emit_num_list = {emit_num_list} emitter_kind = {emitter_kind}")
    ...
    def mid2PMLIn(self,
        ...
        PML = self.mid_symbols.sT["materials"]["PMLSetting"]["parameters"]
        ...
        PMLIn = {
            ...
            "content" : [{
                "sys_type":"ini",
                "key": PML["key"], 
                "powerOrder": PML["powerOrder"],
                "alpha": PML["alpha"], 
                "kappaMax": PML["kappaMax"], 
                "sigmaRatio": PML["sigmaRatio"]
            }]
        }
    ...
    def mid2SpeciesIn(self,
        ...
        electron_paras = self.mid_symbols.sT["physics_entities"]["particle_library"][0]["parameters"]
        ptclCreationRate = self.mid_symbols.sT["global_settings"]["particle_parameters"]["parameters"]["ptclCreationRate"]
        ...
        SpeciesIn = {
            ...
            "content" : [{
                "sys_type":"ini",
                "charge": electron_paras["charge"], 
                "mass": electron_paras["mass"], 
                "ptclCreationRate": ptclCreationRate, 
                "kind": "electron", 
                "name": "Species"
            }]
        }
    ...
    def GetrMax(self):
        ...
        """
        # 获取几何点数据
        points = self.mid_symbols.sT["geometry"]['area_cac_result']['void_area'].get('pnts', [])
    ...
    def mid2StaticNodeFLdsIn(self, rMax: float):
        ...
        """
        if not self.mid_symbols.sT["physics_entities"]["electromagnetic_field"]: return
        ...
        for StaticNodeFLd in self.mid_symbols.sT["physics_entities"]["electromagnetic_field"]:
    ...
    def mid2CircuitModelIn(self,
        ...
        inds = self.mid_symbols.sT["physics_entities"]["inductor"]
        if not inds: return
        print(f"[info] Saving CircuitModelIn")
    ...
    def mid2GlobalSettingIn(self,
        ...
        ureg = UnitRegistry()
        runtime = self.mid_symbols.sT["variable"].get("RUNTIME", {})
        if runtime is None:
            runtime = self.mid_symbols.sT["variable"].get("SIMULATION_TIME", {})
        if runtime != {}:
            runtime_num, runtime_unit = UnitTool.to_default_num(runtime["cac_result"]["var_num"], runtime["cac_result"]["var_unit"])
        ...
        GlobalSettingIn = {
            ...
            "simulationTime": float(runtime_num),
            "EMSolver": "picMode",
