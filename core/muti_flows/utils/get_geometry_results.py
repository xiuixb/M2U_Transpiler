...
class GeomUtils:
    ...
    def geom_vertical_split(self, polygons):
        ...
        if upper_union.is_empty:
            ...
            upper_contour = list(upper_union.exterior.coords)
        else:  # MultiPolygon：取面积最大者的外边界
            largest = max(upper_union.geoms, key=lambda p: p.area)
    ...
    def Cond2Void(self, entities_list)->Tuple[dict, dict]:
        ...
        if current.is_empty:
            ...
            largest = max(current.geoms, key=lambda p: p.area)
